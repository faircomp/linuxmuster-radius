...
for pair in pyproject.toml:requirements.lock build-requirements.in:build-requirements.lock; do
    ...
    canonical="uv pip compile --generate-hashes --python-version=3.12 --output-file=$lock $src"
    bad=0
    ...
    for mode in header target; do
        ...
        (cd "$dir" && uv pip compile --quiet --generate-hashes --python-version=3.12 \
            "${extra[@]}" --output-file="$lock" "$src")
